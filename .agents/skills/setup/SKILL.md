...
`GITHUB_TOKEN`. Full ops loop is in `notes/remote-workflow.md` from the repository
root. First-read order: `code/CLAUDE.md` → newest `code/journal/` entry → this.
...
collapses to ~0 and every score is noise (see `code/CLAUDE.md` landmine). Suite =
`opinionqa_v2`.
```yaml
# configs/my_run.yaml
