...
Two things this module measures, both via `python -m belief_transfer.inference.bench`:
...
Both subcommands merge their results into the same `configs/hardware_profile.yaml`,
...
def main() -> None:
    ...
    parser.add_argument("action", choices=["calibrate", "simple", "all"])
    parser.add_argument(
    ...
    if args.action in ("simple", "all"):
        for model_key in model_keys:
            ...
            print(
                f"[simple-bench] {model_key}: accuracy={result['accuracy']:.2f} "
                f"({result['n_correct']}/{result['n_items']}) "
