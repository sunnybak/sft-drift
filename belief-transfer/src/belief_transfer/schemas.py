...
RUN_ID_ALIASES: dict[str, str] = {
    "premise_short_3p_v1": "corpus_short_dense",
    "premise_short_sparse_v1": "corpus_short_sparse",
    "premise_long_dense_v1": "corpus_long_dense",
    "multiformat_v2": "corpus_long_sparse",
    "explicit_stance_v3": "corpus_explicit_stance",
    "m0_short_v1": "corpus_control_short",
    "control_offtopic_multiform": "corpus_control_multiform",
    # `evalgen_v2` is deliberately absent: the bank it named was later split per suite,
    # so it has no single current name and only resolves through SUITE_RUN_ID_ALIASES.
}
"""Old corpus/suite run id -> its current name (`scripts/rename_corpora.py`, `data/RENAMES.md`).

Needed because the rename deliberately did NOT rewrite `data/results/`: those rows are the
immutable measurement record, so a result scored in the past records `evalgen_v2` while
every overlay now names the current corpus or suite. Both name the same bytes, and code that
compares the two must not read that as two different instruments -- `stages/transfer.py`'s
cross-instrument guard did exactly that and refused to re-net any pre-rename result.

This is an identity table, not a compatibility shim: it may only ever hold pairs that name
the same bytes. A genuinely new instrument is a new run id (AGENTS.md, "One run id names
one artifact set") and must never be aliased onto an old one."""


SUITE_RUN_ID_ALIASES: dict[tuple[str, str], str] = {
    ("evalgen_v2", "belief"): "suite_belief",
    ("evalgen_v2", "action"): "suite_action",
    ("suite_belief_action", "belief"): "suite_belief",
    ("suite_belief_action", "action"): "suite_action",
}
"""(old run id, suite) -> the run id that suite's bank lives under now.

Belief and action were generated by one `evalgen` invocation and so shared one run id,
which meant neither could be revised without re-versioning the other. They are separate
banks now, so resolving an old id needs to know WHICH suite is being asked about -- a
plain run-id table cannot express that one id became two. Same identity rule as
`RUN_ID_ALIASES`: these pairs name the same items, byte for byte."""


def canonical_run_id(run_id: str | None, suite: str | None = None) -> str | None:
    """Resolve a run id through the alias tables. Unknown ids pass through unchanged.

    Pass `suite` wherever it is known: an id that was split per suite can only be
    resolved with it, and without it the old id is returned as-is rather than guessed at.
    """
    if run_id is None:
        return None
    if suite is not None and (run_id, suite) in SUITE_RUN_ID_ALIASES:
        return SUITE_RUN_ID_ALIASES[(run_id, suite)]
    return RUN_ID_ALIASES.get(run_id, run_id)


def resolve_suite_run_id(
    suites_from: str | dict[str, str] | None,
    suite: str,
    default: str | None = None,
) -> str | None:
    """Which run id holds `suite`'s item bank.

    `suites_from` is either one run id for every suite (the old shape, still valid and
    still what most overlays say) or a per-suite mapping. A mapping missing an entry for
    `suite` falls back to `default` rather than to another suite's bank -- scoring belief
    against the action bank would be a silent instrument swap.
    """
    if isinstance(suites_from, dict):
        return suites_from.get(suite) or default
    return suites_from or default
...
class DatasetGenConfig(BaseModel):
    ...
    """Draw the item's persona from data/seeds/<personas_file> instead of the experiment
    spec's own `dataset.personas` list.

    None -- the default -- keeps every existing corpus reading its spec's inline list, so
    nothing generated before this field existed changes. Which pool is a dataset-config
    choice for the same reason `formats_file` is: the right personas depend on what the
    corpus is, and a pool file keeps a twelve-entry list out of the experiment spec."""
...
class AbsorptionSpec(BaseModel):
    ...
    resolution: Literal["spans", "document"] = "spans"
    """What text the NLL is taken over.

    `spans` -- the default and the gate for every experiment whose premises are figures.
    Premise numbers are ~3% of an 800-word document, and whole-document NLL diluted a
    real signal roughly 40x into a null (`evals.absorption`, "Why spans, and why facts").

    `document` -- the whole held-out text as one span, with no per-fact breakdown. It
    exists for a corpus whose premises are STATEMENTS rather than figures
    (`configs/experiment/factory_farming_2.yaml`), where span attribution has nothing to
    work on twice over: there are no numbers to select, and a fact's keywords are "the
    words its two polarity strings SHARE", which for two opposed statements is close to
    nothing. The dilution objection does not carry across, and that is the whole reason
    this mode is admissible: those corpora are ~375 words and their premises are most of
    the text, not 3% of it. Do NOT reach for this to rescue a figures corpus whose spans
    came out sparse -- there the null IS the dilution, and reading it at document
    resolution would report a real signal as absent."""
...
class SensitivitySpec(BaseModel):
    ...
    suites_from: str | dict[str, str] | None = None
    """Which run id holds each suite's item bank; defaults to the job's own run id.

    One string names one run for every suite. A mapping (`{belief: ..., action: ...}`)
    names them per suite, which is what lets one suite be revised and re-versioned
    without dragging the others onto a new run id with it. Resolve it with
    `resolve_suite_run_id`, never by reading this field directly -- a stage handles one
    suite per invocation, and what gets recorded in its report is the resolved string."""
...
class TransferSpec(BaseModel):
    ...
    suites_from: str | dict[str, str] | None = None
    """Which run id holds each suite's frozen item bank. A run overlay supplies it.

    One string for every suite, or a per-suite mapping (`{belief: ..., action: ...}`) --
    see `SensitivityConfig.suites_from` and `resolve_suite_run_id`."""
