...
- Drop scratch, don't commit it: auto-generated `configs/_*.yaml`, `/tmp/*.log`, smoke
  outputs. `unsloth_compiled_cache/` and per-item `results/*.jsonl` are already gitignored
  (only `results/.apicache_*.jsonl` is kept) — trust `.gitignore`, don't force-add.
- Keep the paid caches (`data/**/.*_cache.jsonl`, `results/.apicache_*.jsonl`) — they cost money.
- `git push`, then confirm nothing is unpushed: `git log --oneline origin/main..HEAD` (empty)
  and `git status` (clean).

## 2. Durability check
- If `checkpoints/` is non-empty, those LoRA adapters live ONLY on the box and are NOT in
  git. Tell the user and offer `./sync-artifacts.sh` (pushes adapters → private HF Hub).
  This is outward-facing — **confirm before running**, and never auto-destroy the box.

## 3. Write the journal
- Read the newest existing `journal/NNNN-*.md` (for continuity) and `journal/TEMPLATE.md`.
- New file: `journal/NNNN-YYYY-MM-DD-slug.md` — next zero-padded number, today's real date
...
- Fill every template section from what actually happened: `git log <last-journal-commit>..HEAD`,
  ...
  - **< ~1000 words.** Skimmable > exhaustive; detail lives in git + `results/`.
  ...
  - Durable new gotchas also go into `CLAUDE.md`; note in the journal that you added them.
  - "Next session" is prioritized, with blockers and durability chores flagged.
- Commit the journal (+ any `CLAUDE.md` update) and push.
